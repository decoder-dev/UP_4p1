...
{
    internal class Filter_med_cart_healtGroup
    {
        public static void Filter(string Group)
        {
            try
            {
                string sql = @"SELECT medical_card.Kod_medical_card,kid.Name,kid.Surname,kid.Patronymic,medical_card.Height,medical_card.Weight,health_group.Group,medical_card.Chronic_diseases
                           ...
                           AND medical_card.Kod_health_group = health_group.Kod_health_group AND health_group.Group='" + Group + "';";
